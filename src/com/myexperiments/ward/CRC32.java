...
/******************************************************************************
 * Compilation: javac CRC32.java Execution: java CRC32 s
 * 
 * Reads in a string s as a command-line argument, and prints out its 32 bit
 * Cyclic Redundancy Check (CRC32 or Ethernet / AAL5 or ITU-TSS).
 *
 * Uses direct table lookup, calculation, and Java library.
 *
 * % java CRC32 123456789 CRC32 (via table lookup) = cbf43926 CRC32 (via direct
 * calculation) = cbf43926 CRC32 (via Java's library) = cbf43926
 *
 *
 *
 * Uses irreducible polynomial: 1 + x + x^2 + x^4 + x^5 + x^7 + x^8 + x^10 +
...
public class CRC32 {
	...
	/*
	 * private long getStreamCheckSum(InputStream in) throws Exception { CRC32 sum =
	 * new CRC32();
	 * 
	 * byte[] buf = new byte[32 * 1024];
	 * 
	 * for (;;) { int read = in.read(buf); if (read == -1) { break; }
	 * sum.update(buf, 0, read); } in.close(); return sum.getValue(); }
	 * 
	 * private void verify(CRC32 crc) throws IOException { try (DataInputStream in =
	 * new DataInputStream(new ByteArrayInputStream(_trailerBytes))) { long crc32 =
	 * readLittleEndianLong(in); if (crc32 != crc.getValue()) { throw new
	 * IOException("crc32 mismatch. Gzip-compressed data is corrupted"); } long
	 * isize = readLittleEndianLong(in); if (isize != (inf.getBytesWritten() &
	 * SIZE_MASK)) { throw new
	 * IOException("Uncompressed size mismatch. Gzip-compressed data is corrupted");
	 * } } }
	 */
