...
public class TestFusionDocumentWriter {
	...
	static {
		...
		try (InputStream in = new FileInputStream("properties/propertiesfusion.xml")) {
			...
			fusionIndexingPipelineUrlExtension = prop.getProperty("fusionIndexingPipelineUrlExtension",
					defaultFusionIndexingPipelineUrlExtension);
			...
			fusionSolrProxyUrlExtension = prop.getProperty("fusionSolrProxyUrlExtension",
					defaultFusionSolrProxyUrlExtension);
		} catch (IOException e) {
	...
	@Rule
	public WireMockRule wireMockRule = new WireMockRule(Integer.parseInt(wireMockRulePort)); // No-args constructor
	...
	public void testFusionDocumentWriter() throws Exception {
		...
		String fusionSolrProxyWithoutHostAndPort = fusionProxyBaseUrl + fusionSolrProxyUrlExtension
		...
		if (useWireMockRule) {
			...
			// register a callback to validate the request that came into our mock pipeline
			// endpoint
		...
		testDocsSentMeter = docWriter.getMetricRegistry()
				.meter(FusionDocumentWriter.metricName(FusionDocumentWriter.class, "Test Docs", fusionCollection));
	...
	/**
	 * Verify the JSON document that was posted to the pipeline by the
	 * FusionDocumentWriter.
	 */
		...
		if (request.getUrl().endsWith(defaultFusionIndexingPipelineUrlTerminatingString)) {
			...
			try {
				...
				if (!tree.isArray() || tree.size() != 1)
					fail("Expected JSON list containing one object in request to the pipeline endpoint, but got: "
							+ tree);
			...
			// TODO The validation of a solr input document needs to be improved...currently
			// there is no checking only an
			// additional 'else' clause to detect documents being sent to Solr instead of
			// the Fusion pipeline. Without
	...
	/**
	 * shs: Generate atomic update documents for validating the atomic update
	 * portion of the add method.
	 * 
	 * @param numDocs
	 *            The number of atomic update documents to be created.
	 * @param startVal
	 *            The initial starting value of incrementer. By having this
	 *            parameter, we can create lists of documents where normal documents
	 *            and atomic update documents are intermingled. This will permit the
	 *            verification of the parser that seperates atomic update documents
	...
	/**
	 * shs: Generate nested documents for validating the nested document portion of
	 * the add method.
	 * 
	 * @param numDocs
	 *            The number of atomic update documents to be created.
	 ...
	 *            verification of the parser that seperates atomic update documents
	 *            from regular documents.
	 * @TODO This would be better with a generalized processor that would create
	 *       nested documents to a specific nesting depth. Currently any number of
	 *       documents will be created; however, those documents will follow this
	 *       pattern: Odd numbered document will have a parent document with three
	 *       children. The third child also has three children. Even numbered
	 *       documents have a parent document with two child documents.
	 * @return
	 */
	protected Map<String, SolrInputDocument> buildNestedDocs(int numDocs, int startVal) {
		...
		for (int d = startVal; d < numDocs + startVal; d++) {
			...
			/***
			 * Equivalent Solr XML doc: <add> <doc> <field name="id">product01</field>
			 * <field name="name_s">car</field> <field name="content_type_s">product</field>
			 * <doc> <field name="id">part01</field> <field name="name_s">wheels</field>
			 * <field name="content_type_s">part</field> </doc> <doc>
			 * <field name="id">part02</field> <field name="name_s">engine</field>
			 * <field name="doctype">part</field> </doc> <doc>
			 * <field name="id">part03</field> <field name="name_s">brakes</field>
			 * <field name="content_type_s">part</field> <doc>
			 * <field name="id">subpart01</field> <field name="name_s">brakePads</field>
			 * <field name="content_type_s">component</field> </doc> <doc>
			 * <field name="id">subpart02</field> <field name="name_s">calipers</field>
			 * <field name="content_type_s">component</field> </doc> <doc>
			 * <field name="id">subpart03</field> <field name="name_s">drums</field>
			 * <field name="content_type_s">component</field> </doc> </doc> </doc> <doc>
			 * <field name="id">product02</field> <field name="name_s">truck</field>
			 * <field name="content_type_s">product</field> <doc>
			 * <field name="id">part04</field> <field name="name_s">wheels</field>
			 * <field name="content_type_s">part</field> </doc> <doc>
			 * <field name="id">part05</field> <field name="name_s">flaps</field>
			 * <field name="doctype">part</field> </doc> <doc> </add>
			 ***/
