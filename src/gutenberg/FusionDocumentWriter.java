...
public class FusionDocumentWriter implements SolrInputDocumentWriter {
	...
	public FusionDocumentWriter(String indexName, Map<String, String> connectionParams) {
		...
		if (fusionEndpoint == null)
			throw new IllegalStateException(
					"The 'fusion.pipeline' parameter is required when using Lucidworks Fusion!");
		...
		if (fusionSolrProxy == null)
			throw new IllegalStateException(
					"The 'fusion.solrproxy' parameter is required when using Lucidworks Fusion!");
	...
	public void add(int shard, Map<String, SolrInputDocument> inputDocumentMap)
		...
		// passed in via the inputDocumentMap.
		List<SolrInputDocument> inputDocuments;
		try {
			inputDocuments = addAtomicUpdateDocuments(inputDocumentMap.values());
			// Catch the exception so that any remaining documents in the collection will be
			// able to be processed and submitted
		...
		// shs: Only process the inputDocuments collection if there are still documents
		// remaining in the collection after
		...
		if (inputDocuments != null && !inputDocuments.isEmpty()) {
			...
			if (numFusionDocsRcvd > 0) {
				...
				try {
					...
					log.warn("FusionPipelineClient failed to process batch of " + numFusionDocsRcvd + " docs due to: "
							+ e + "; will re-try each doc individually");
	 ...
	 *            This is the list of documents that are atomic update documents.
	 * @return inputDocuments (modified: the atomic update documents have been
	 *         removed from the incoming parameter and only non-atomic update
	 *         documents remain in that collection). The null value will be returned
	 *         if all documents from the list of SolrInputDocuments have been
	 *         removed.
	...
	protected List<SolrInputDocument> addAtomicUpdateDocuments(Collection<SolrInputDocument> inputDocuments)
			throws SolrServerException, IOException {
		// Visit each document in the collection and determine if it is a document that
		// is an atomic update request. If it
		...
		while (docIterator.hasNext()) {
			...
			for (SolrInputField solrInputField : doc.values()) {
				...
				// If the type of the field just retrieved from the document is a Map object,
				// then this could be an atomic
				// update document.
				if (val instanceof Map) {
					int entryCount = 0; // Used only for log messages. If the log message below is removed, this may
										// also be deleted.
					for (Map.Entry<String, Object> entry : ((Map<String, Object>) val).entrySet()) {
						String key = entry.getKey();
						if (key.equals("add") || key.equals("set") || key.equals("remove") || key.equals("removeregex")
								|| key.equals("inc")) {
				...
				// The document was determined to be an atomic update document, no need to visit
				// the rest of the fields, so
		...
		// The following processing is necessary IFF there are documents in the
		// atomicUpdateDocuments collection, which
		// means that we have documents needing an atomic update.
		if (atomicUpdateDocuments != null && !atomicUpdateDocuments.isEmpty()) {
			// For Fusion document submission, the SolrInputDocument must be converted to
			// JSON; however, since these
			// documents are going directly to Solr and are NOT being submitted to a Fusion
			// indexing pipeline, they do not
			...
			try {
				...
				log.warn("Solr failed to process batch of " + atomicUpdateDocuments.size() + " atomic updates due to: "
						+ e + "; will re-try each doc individually");
				retrySolrAtomicUpdatesIndividually(atomicUpdateDocuments);
			}
		}
		// Finally, return any documents remaining in the inputDocuments collection for
		// processing through the Fusion
		// indexing pipeline.
		return retain;
	}

	/**
	 * shs: This method was modified to add the parent and docCount parameters.
	 * These were added to support the recursion needed to flatten (aka denormalize)
	 * the parent/child document to any nesting level the input document may have
	 * been nested to.
	 * 
	 * @param parentSolrDoc
	 *            This is the parent document of the list of childSolrDocs. It is in
	 *            SolrInputDocument format. This parameter will be 'null' when first
	 *            called. When this method is called recursively, it will not be
	 *            null.
	 * @param childSolrDocs
	 *            This is the list documents that are the children (nested)
	 *            documents of parentSolrDoc.
	 * @param docCount
	 *            This is the number of child documents for a parent. It is used as
	 *            the child ID
	 * @return The list documents to be sent to Fusion for indexing formatted as
	 *         JSON documents.
	 * @throws Exception
	 */
	...
	/**
	 * shs: This method was modified from its original to add the input parameters
	 * 'parent' and 'docCount'. This was done to enable recursion to be used to find
	 * all parent/child relationships to any level.
	 * 
	 * @param parent
	 *            The parent document for the child document being passed in. Parent
	 *            may be null if the child being passed in is a member of the
	 *            initial documents submitted.
	 * @param child
	 *            This is the child document. This document will be examined to
	 *            determine if there are nested child documents in it.
	 * @param docCount
	 *            This is the number of child documents for a parent. It is used as
	 *            the child ID
	 * @return The list of JSON formatted documents, denormalized (aka. flattened)
	 *         and ready to be send to the Fusion indexing pipeline endpoint.
	 * @throws Exception
	 */
	 ...
	 * shs: This method was modified from its original to add the input parameters
	 * 'parent' and 'docCount'. This was done to enable recursion to be used to find
	 * all parent/child relationships to any level. The method will merge the fields
	 * in the parent document into the child document and will then convert that
	 * merged document into JSON format and return that JSON document to the caller.
	 * 
	 * @param parent
	 *            The parent document for the child document being passed in. Parent
	 *            may be null if the child being passed in is a member of the
	 *            initial documents submitted.
	 * @param child
	 *            This is the child document. It will have the parent's fields
	 *            merged into it.
	 * @param docCount
	 *            This is a count of the number of documents that have been added in
	 *            this processing.
	 * @return The merged parent and child documents as a JSON formatted document,
	 *         in a format acceptable to Fusion.
	 */
	protected Map<String, Object> doc2json(SolrInputDocument parent, SolrInputDocument child, int docCount) {
		...
		if (child != null) {
			...
			if (parent != null) {
				...
				// have a parent doc ... flatten by adding all parent doc fields to the child
				// with prefix _p_
				for (String f : parent.getFieldNames()) {
					if ("id".equals(f)) {
						fields.add(
								mapField("_p_id", null /* field name prefix */, parent.getField("id").getFirstValue()));
					} else {
	...
	/**
	 * shs: This method was added to handle documents that are submitted as atomic
	 * updates. It is the final option to get documents into the index if the
	 * previous indexing attempt failed when sending the entire colleciton of
	 * documents to the Solr proxy. In this case, each document in the collection of
	 * documents will be submitted to the Solr proxy, one document at a time.
	 * 
	 * @param docs
	 *            The SolrInputDocuments to be added, one at a time.
	 * @throws SolrServerException
	 * @throws IOException
	 */
	private void retrySolrAtomicUpdatesIndividually(Collection<SolrInputDocument> docs)
		...
		for (SolrInputDocument nextDoc : docs) {
			try {
				...
				log.error("Failed to index atomic update document [" + nextDoc.get("id") + "] due to: " + e + "; doc: "
						+ nextDoc + "solrProxy:[" + solrProxy.toString() + "]");
	...
	public void deleteById(int shard, List<String> idsToDelete) throws SolrServerException, IOException {
		...
		String listLogInfo = (idsToDelete.size() > len)
				? (idsToDelete.subList(0, len).toString() + " + " + (idsToDelete.size() - len) + " more ...")
				: idsToDelete.toString();
		...
		try {
			...
			// This statement was inserted for Zendesk ticket 4186. If the delete by IDs
			// succeeds above, we also need to ensure
			// that all children documents that have the id (HBase row ID) in their id
			// followed immediately by the
			// deleteByQueryAppendString followed by any additional characters are also
	...
	/**
	 * If deleting the documents using a list of document IDs fails, then retry the
	 * deletes while iterating through the list of document IDs to be deleted. As
	 * the result of changes for ticket 4186, a delete by ID will also result in a
	 * delete by query attemp, where the id is postpended by a specific string. In
	 * the situation where there were nested (parent/child) documents submitted for
	 * indexing, the documents were denormalized and when this happens the document
	 * ID of the denormalized child documents contains the ID of the parent (the
	 * HBase row ID) followed by a specificpattern of characters followed by an
	 * additional string that uniquely identifies the child document. The delete by
	 * query will take the parent ID (HBase row ID) and append to it the specific
	 * pattern of characters followed by the wild card character ('*'). This will
	 * result in all child documents of the parent being deleted from the index.
	 *
	 * @param idsToDelete
	 *            The ID (HBase row ID) of the documents to be deleted
	 * @param retryDeletesByQueryOnly
	 *            If the delete by IDs succeeded and only the delete by queries
	 *            failed, this will be 'true' to indicate that only the delete by
	 *            query should be retried. If the value is 'false', the delete by id
	 *            failed, meaning that the delete by query was never attempted;
	 *            thus, both the delete by id AND the delete by query must be
	 *            attempted.
	 * @throws SolrServerException
	 * @throws IOException
	 */
	private void retryDeletesIndividually(List<String> idsToDelete, boolean retryDeletesByQueryOnly)
			throws SolrServerException, IOException {
		for (String idToDelete : idsToDelete) {
			if (!retryDeletesByQueryOnly) {
				try {
					...
					log.error("Failed to delete document with ID " + idToDelete + " due to: " + e
							+ ". Retry deleting by query as '" + idToDelete + deleteByQueryAppendString + "'");
				}
			}
			// Try to delete the document by query so as to remove all child documents from
			// the index.
			try {
				deleteByQuery(idToDelete + deleteByQueryAppendString);
			} catch (SolrException e) {
				log.error("Failed to delete document by query inside method 'retryDeletesIndividually' with ID "
						+ idToDelete + deleteByQueryAppendString + " due to: " + e + ".");
			}
		}
	}
	...
	// private void deleteByQuery(String idToDelete, String queryFieldName, String
	// deleteQueryAppendStr) throws SolrServerException, IOException {
	// try {
	// deleteByQuery(queryFieldName + ":" + idToDelete + deleteQueryAppendStr);
	// } catch (Exception e) {
	// log.error("Failed to execute deleteByQuery(String idToDelete, String
	// deleteQueryAppendStr): " + idToDelete + deleteQueryAppendStr + " due to: " +
	// e);
	// }
	// }

	private void deleteByQuery(List<String> idsToDelete, String queryFieldName, String deleteQueryAppendStr)
			throws SolrServerException, IOException {
		for (String idToDelete : idsToDelete) {
			try {
				deleteByQuery(queryFieldName + ":" + idToDelete + deleteQueryAppendStr);
			} catch (Exception e) {
				log.error("Failed to execute deleteByQuery(List<String> idsToDelete, String deleteQueryAppendStr): "
						+ idToDelete + deleteQueryAppendStr + " due to: " + e);
			}
		}
	}
